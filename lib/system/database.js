...
try {
  ...
  console.error('خطأ أثناء تحميل قاعدة البيانات:', error);
}
...
export function getUser(id, opt = {}) {
  if (!id) {
    ...
    if (orderBy) {
      ...
      if (!allowedCols.includes(orderBy)) {
        throw new Error('العمود غير مسموح به');
      }
...
export function updateChatUser(
  chatId,
  userId,
  field,
  val
...
export function getStickersPack(id) {
  if (!id) {
    return Object.values(
      database.sticker_packs
    );
  }

  if (!database.sticker_packs[id]) {
    database.sticker_packs[id] = {
      ...defStickerPack,
      id
    };
...
export function updateStickersPack(
  id,
  field,
  val
) {
...
export function setCreate(
  ...
  if (table === 'chat_users') {
    if (
      ...
      throw new Error(
        'chat_users يحتاج [chatId, userId]'
      );
    }
