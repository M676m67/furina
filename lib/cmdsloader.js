...
function registerModule(filePath, mod) {
  ...
  if (!plugin) {
    console.error(
      chalk.red(
        `[ Furina ] ${path.basename(filePath)} does not contain export default`
      )
    );
 ...
 const allFn =
  ...
  if (typeof plugin.run !== 'function') {
    ...
    console.log(
      chalk.gray(
        `[ Furina ] ${path.basename(filePath)} loaded without command`
      )
    );
  ...
  if (!commands.length) {
    ...
    console.log(
      chalk.gray(
        `[ Furina ] Plugin: ${path.basename(filePath)} · Without command`
      )
    );
...
async function scan(directory) {
  ...
  for (const filePath of files) {
    ...
    try {
      ...
      console.error(
        chalk.red(
          `[ Furina ] Error loading ${path.basename(filePath)}:`
        )
      );
...
async function reloadFile(filePath) {
  ...
  if (!fs.existsSync(filePath)) {
    ...
    console.log(
      chalk.gray(
        `[ Furina ] Plugin deleted: ${path.basename(filePath)}`
      )
    );
  ...
  try {
    ...
    console.log(
      chalk.green(
        `[ Furina ] ✓ Updated: ${path.basename(filePath)}`
      )
    );
    ...
    console.error(
      chalk.red(
        `[ Furina ] ✗ Update failed for ${path.basename(filePath)}`
      )
    );
...
function watchDir(directory) {
  ...
  try {
    ...
    console.error(
      chalk.red(
        `[ Furina ] Unable to watch ${directory}: ${error.message}`
      )
    );
...
export default async function cmdsLoader() {
  ...
  console.log(
    chalk.gray(
      `[ Furina ] Loaded ${global.comandos.size} commands from ${total} Plugins in ${time}ms${
        errors
          ? ` · ${errors} errors`
          : ''
      }`
