...
╰${lineM}\n${chalk.bold.magentaBright("---> ")}`);
  ...
  while(!/^[1-2]$/.test(opcion)){
    console.log(chalk.bold.redBright("Numbers other than 1 or 2, and special letters or symbols are not allowed.."));
...
async function warmupGroups(furina){
  try{
    ...
    console.log(chalk.gray(`[ ✿ ] Data loading ${chatIds.length} Pre-set...`));
    const t=Date.now();
    const batches=[];
    ...
    console.log(chalk.gray(`[ ✿ ] Preloading completed within ${Date.now()-t}ms`));
  }catch(e){
...
export async function startBot(){
  ...
  furina.ev.on("creds.update",saveCreds);

  furina.sendText=(jid,text,quoted="",options)=>furina.sendMessage(jid,{text,...options},{quoted});
    ...
    setTimeout(async()=>{
      try{
        if(!state.creds.registered){
          const pairing=await furina.requestPairingCode(phoneNumber);
          const codeBot=pairing?.match(/.{1,4}/g)?.join("-")||pairing;
          console.log(chalk.bold.white(chalk.bgMagenta("Pairing code:")),chalk.bold.white(chalk.white(codeBot)));
        }
      }catch(err){
        console.log(chalk.red("Error while generating the code:"),err);
      }
    },3000);
  }

  furina.ev.on("messages.upsert",async({messages,type})=>{
    if(!botReady)return;
    ...
    for(const msg of messages){
      ...
      try{
        ...
        const m=await smsg(furina,msg);

        if(typeof handler==="function"){
          handler(furina,m,messages).catch(err=>console.error("[ ✿ ] Main account »",err?.message));
        }
      }catch(err){
  ...
  try{
    await events(furina,null);
  }catch(err){
    console.log(chalk.gray(`[ Events Error ] → ${err}`));
  }
  ...
  furina.ev.on("connection.update",async update=>{
    ...
    if(qr!=0&&qr!=undefined||methodCodeQR){
      if(opcion=="1"||methodCodeQR){
        console.log(chalk.green.bold("[ ✿ ] Scan this QR code"));
    ...
    if(connection==="open"){
      ...
      const userName=furina.user.name||"Unknown";
      log.success(`Connected to: ${userName}`);
    ...
    if(isNewLogin)log.info("New device detected");

    if(receivedPendingNotifications===true){
      log.warn("Please wait about one minute...");
      furina.ev.flush();
    }
    ...
    if(connection==="close"){
      ...
      if([DisconnectReason.loggedOut,DisconnectReason.forbidden,DisconnectReason.multideviceMismatch].includes(reason)){
        log.warn(`Main account disconnected (${reason}) — cleaning session and restarting...`);
        botReady=false;
        isRestarting=false;
        clearSession();
        process.exit(1);
      ...
      if(reason===DisconnectReason.connectionReplaced){
        log.warn("Connection replaced — close the other session before reconnecting.");
        isRestarting=false;
        return;
      }
      ...
      if(reconexion>retriesLimit){
        log.error(`Reconnect attempt limit (${retriesLimit}) exceeded — the session may be corrupted, cleaning it...`);
        botReady=false;
        reconexion=0;
        isRestarting=false;
        clearSession();
      ...
      const reasonMessages={
        [DisconnectReason.connectionLost]:"Connection to the server was lost, attempting to reconnect...",
        [DisconnectReason.connectionClosed]:"Connection closed, attempting to reconnect...",
        [DisconnectReason.restartRequired]:"Connection restart required...",
        [DisconnectReason.timedOut]:"Connection timed out, attempting to reconnect...",
        [DisconnectReason.badSession]:"Invalid session, cleaning it and reconnecting..."
      };

      log.warn(reasonMessages[reason]||`Connection closed (${reason}), reconnecting in ${delay/1000} seconds...`);
    ...
    async update => {
      ...
      if (
        ...
        if (
          ...
          log.warn(
            `The main account was disconnected (${reason}) — cleaning the session and restarting...`
          );
        ...
        if (
          ...
          log.warn(
            "The connection was replaced — close the other session before reconnecting."
          );
        ...
        if (
          ...
          log.error(
            `The maximum number of reconnection attempts (${retriesLimit}) has been exceeded — the session may be corrupted, cleaning it up...`
          );
        ...
        const reasonMessages = {
          ...
          [DisconnectReason.connectionClosed]:
            "The connection was closed, attempting to reconnect...",

          [DisconnectReason.restartRequired]:
            "The connection needs to be restarted...",

          [DisconnectReason.timedOut]:
            "The connection timed out, attempting to reconnect...",

          [DisconnectReason.badSession]:
            "The session is invalid, cleaning it up and reconnecting..."
        };

        log.warn(
          reasonMessages[reason] ||
          `The connection was interrupted (${reason}), reconnecting in ${
            delay / 1000
          } seconds...`
        );
